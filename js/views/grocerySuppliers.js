...
export async function renderGrocerySuppliersView(container) {
            ...
            <tr>
              ...
              <td class="text-center">
                <div style="display:flex;gap:4px;justify-content:center">
                  <button class="btn btn-sm btn-warning btn-add-bill" data-id="${s.id}" title="Add Bill">
  ...
  `;
    ...
    // Add Bill
    container.querySelectorAll('.btn-add-bill').forEach(btn => {
        btn.addEventListener('click', async () => {
            const s = await DB.getById('grocerySuppliers', parseInt(btn.dataset.id));
            if (s) showBillForm(s, container);
        });
    });
